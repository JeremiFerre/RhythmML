...
public class SoundManager
{
     ...
     */
    public int addClip(String filePath)
     ...
     * @throws LineUnavailableException
     */
    public int addClip(String filePath, int start, int end)
        throws IOException, UnsupportedAudioFileException, LineUnavailableException
    {
        AudioInputStream audioInputStream = AudioSystem.getAudioInputStream(new File(filePath));
        ...
        byte[] audioSample = new byte[size+1];
        audioSample = Arrays.copyOfRange((byte[])audio, start, end);
        afs.add(af);
        sizes.add(new Integer(audioSample.length));
        infos.add(info);
     ...
     * @throws LineUnavailableException
     */
    public int addClip(String filePath, int sampleSize)
            throws IOException, UnsupportedAudioFileException, LineUnavailableException
        {
            AudioInputStream audioInputStream = AudioSystem.getAudioInputStream(new File(filePath));
            AudioFormat af = audioInputStream.getFormat();
...
//    }
    ...
    public int addClipFromClipConcat(int s1, int s2, int inBetweenSilenceSize) {
    	...
    	AudioFormat af = afs.get(s1); //arbitrarily they should be the same anyway.
        Info info = new Info(Clip.class, afs.get(s1), size);
     ...
     * @throws LineUnavailableException
     */
    public void playSound(int x, int loop)
          throws UnsupportedAudioFileException, LineUnavailableException
    {
            ...
            else
            {
                  ...
                  clip.open(afs.elementAt(x), audios.elementAt(x), 0, sizes.elementAt(x).intValue());
                  clip.loop(loop);
            }
      }
